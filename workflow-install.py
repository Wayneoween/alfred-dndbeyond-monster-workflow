...
DEFAULT_DIRS = (
    os.path.expanduser('~/Library/Application Support/Alfred'),
    os.path.expanduser('~/Library/Application Support/Alfred 3'),
)
...
def get_workflow_directory():
    ...
    default_syncdir = None
    syncdir = None
    for i, path in enumerate(ALFRED_PREFS):
        if os.path.exists(path):
            log.debug('looking for preferences file %r ...', path)
            prefs = read_plist(path)
            syncdir = prefs.get('syncfolder')
            default_syncdir = DEFAULT_DIRS[i]
            break
    ...
    if not syncdir:
        ...
        if not syncdir or not os.path.exists(syncdir):
            syncdir = default_syncdir
    ...
    if not os.path.exists(syncdir):
        log.debug('Alfred syncdir %r does not exist', syncdir)
        return None
